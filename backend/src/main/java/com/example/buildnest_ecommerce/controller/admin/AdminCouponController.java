...
public class AdminCouponController {
    ...
    @GetMapping
    @Auditable(action = "ADMIN_LIST_COUPONS", entityType = "COUPON")
    public ResponseEntity<ApiResponse> getAllCoupons() {
        try {
            List<Coupon> coupons = couponService.getAllCoupons();
            return ResponseEntity.ok(
                    new ApiResponse(
                            true, "Coupons retrieved successfully", coupons));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse(
                            false, "Error retrieving coupons", null));
        }
    }
    ...
    public ResponseEntity<ApiResponse> createCoupon(
        ...
        try {
            Coupon coupon = couponService.createCoupon(
                    request.getCode(), request.getDiscountType(),
                    request.getDiscountValue(), request.getMinOrderValue(),
                    request.getUsageLimit(), request.getExpiresAt());
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(new ApiResponse(
                            true, "Coupon created successfully", coupon));
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse(
                            false, "Error creating coupon", null));
        }
    }

    @DeleteMapping("/{id}")
    @Auditable(action = "ADMIN_DEACTIVATE_COUPON", entityType = "COUPON")
    public ResponseEntity<ApiResponse> deactivateCoupon(
            @PathVariable Long id) {
        try {
            Coupon coupon = couponService.deactivateCoupon(id);
            return ResponseEntity.ok(new ApiResponse(
                    true, "Coupon deactivated successfully", coupon));
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse(
                            false, "Error deactivating coupon", null));
        }
    }
}
