...
public class ProductControllerV2 {
        ...
        @Operation(summary = "Get all products with pagination",
                   description = "Returns paginated list of products with "
                                   + "full details wrapped in ApiResponse")
        @ApiResponses({
        ...
        @Operation(summary = "Get product by ID",
                   description = "Retrieves detailed information for a "
                                   + "specific product")
        @ApiResponses({
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(
        ...
        /**
         * Fire-and-forget page-view tracking (ANL-02, #65). Resolves
         * the current user from the security context directly rather
         * than adding an {@code Authentication} parameter, so anonymous
         * views (this endpoint is public) are recorded with a null
         * userId instead of failing/erroring.
         */
        ...
        @Operation(summary = "Search products with advanced filters",
                   description = "Full-text search via Elasticsearch when "
                                   + "enabled; falls back to JPA otherwise.")
        @GetMapping("/search")
        public ResponseEntity<ApiResponse> searchProducts(
                        @Parameter(description = "Search query",
                        example = "cement")
                        @RequestParam(required = false) String query,
                        @Parameter(description = "Category ID", example = "5")
                        ...
                        @Parameter(description = "District ID (FR-LOC-03)",
                        example = "12")
                ...
                if (productSearchService.isPresent()) {
                        ...
                        inStock, tag, districtId, pageable);
                        return ResponseEntity.ok(
                        new ApiResponse(true,
                        "Products search completed", results));
                }

                // districtId filtering is Elasticsearch-only for this issue
                // (FR-LOC-03, #563) — the JPA fallback path used when
                // elasticsearch.enabled=false is unaffected; out of scope
                // per the issue's own text ("existing Elasticsearch-backed
                // product search"), not a missed discovery.
        ...
        @Operation(summary = "Get products by category",
                   description = "Retrieves all products in a specific "
                                   + "category")
        ...
        public ResponseEntity<ApiResponse> getProductsByCategory(
                        ...
                        example = "20")
                        @RequestParam(defaultValue = "20") int size) {
                // 2.4 MEDIUM - Pagination Best Practices: max page size
