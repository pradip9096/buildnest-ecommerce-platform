...
public class AuthController {
    ...
    @Operation(summary = "Login",
            description = "Authenticate user; access/refresh tokens are "
    ...
    public ResponseEntity<ApiResponse> login(
        ...
        if (!rateLimitUtil.isAllowed(request, "login")) {
            long retryAfter =
                    rateLimitUtil.getRetryAfterSeconds(request, "login");
        ...
        try {
            ...
            AuthResponse authResponse = authService.login(
                    ...
                    loginRequest.getTotpCode());
            if (authResponse.isTwoFactorRequired()) {
                // No cookies set — credentials verified but a
                // TOTP/recovery code is still required before tokens
                // are issued (#91).
                return ResponseEntity.ok()
                        .body(new ApiResponse(true,
                                "Two-factor authentication code required",
                                AuthUserResponse.from(authResponse)));
            }
            ResponseCookie accessCookie = authCookieService
                    .buildAccessTokenCookie(authResponse.getAccessToken());
            ResponseCookie refreshCookie = authCookieService
                    .buildRefreshTokenCookie(authResponse.getRefreshToken());
            return ResponseEntity.ok()
    ...
    public ResponseEntity<ApiResponse> register(
        ...
        try {
            log.info("Registration attempt for user: {}",
    ...
    public ResponseEntity<ApiResponse> refreshToken(
        ...
        try {
            ...
            if (!rateLimitUtil.isAllowed(request, "refresh", userId)) {
                long retryAfter = rateLimitUtil
                        .getRetryAfterSeconds(request, "refresh", userId);
                return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS)
                        .header("Retry-After", String.valueOf(retryAfter))
                        .body(new ApiResponse(false,
                                "Too many refresh token requests. "
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(new ApiResponse(false,
                            "Token refresh failed: " + e.getMessage(), null));
        }
    }
    ...
    @ApiResponses(value = {
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "500", description = "Logout failed")
    })
    public ResponseEntity<ApiResponse> logout(
        ...
        ResponseCookie clearRefreshCookie =
                authCookieService.clearRefreshTokenCookie();
        try {
            ...
            // Always clear cookies client-side, even if server-side
            // revocation failed — the browser must never keep hold of
            // a stale token after a logout attempt.
    ...
    @GetMapping("/csrf")
    @Operation(summary = "Bootstrap CSRF token",
            description = "Forces resolution of the CSRF token, "
                    + "triggering the XSRF-TOKEN cookie to be set. "
                    + "Call once at app startup before any mutating "
                    + "request.")
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "204", description = "CSRF cookie set")
    })
    public ResponseEntity<Void> csrf(CsrfToken csrfToken) {
        // Touching the token forces CsrfFilter to resolve and set the
        // XSRF-TOKEN cookie.
        csrfToken.getToken();
