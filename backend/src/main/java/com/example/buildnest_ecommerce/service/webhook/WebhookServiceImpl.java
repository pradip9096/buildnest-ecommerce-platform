...
public class WebhookServiceImpl implements WebhookService {
    ...
    @Override
    @Transactional
    public WebhookSubscriptionResponse createSubscription(
            WebhookSubscriptionRequest request) {
        ssrfUrlValidator.validate(request.getTargetUrl());
    ...
    public void dispatchEvent(String eventType, Map<String, Object> payload) {
        List<WebhookSubscription> subscriptions =
                repository.findByEventTypeAndActiveTrue(eventType);
    ...
    private void deliverWithRetries(
            WebhookSubscription subscription, Map<String, Object> payload) {
        int attempt = 0;
        ...
        while (attempt < MAX_RETRIES && !delivered) {
            ...
            try {
                ...
                log.warn("Webhook delivery failed (attempt {} of {}) for "
                                + "subscription {}",
                        attempt, MAX_RETRIES, subscription.getId(), ex);
    ...
    private void deliver(WebhookSubscription subscription,
        ...
        if (subscription.getSecret() != null
                && !subscription.getSecret().isBlank()) {
            headers.set("X-Webhook-Signature",
                    generateSignature(body, subscription.getSecret()));
        }

        HttpEntity<String> request = new HttpEntity<>(body, headers);
        restTemplate.postForObject(
                subscription.getTargetUrl(), request, String.class);
    }
    ...
    private void updateFailure(
            WebhookSubscription subscription, String status) {
    ...
    private WebhookSubscriptionResponse toResponse(
        ...
        return new WebhookSubscriptionResponse(
                ...
                subscription.getFailureCount() == null
                        ? 0 : subscription.getFailureCount(),
