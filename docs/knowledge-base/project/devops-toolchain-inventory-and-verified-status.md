...
| SonarQube / SonarCloud | `security.yml` (`code-quality` job) | ✅ Fixed 2026-07-12 | Was completely non-functional (#350) — fixed and confirmed via a real SonarCloud dashboard with real findings (#351) |
...
| CheckStyle | `security.yml` (`code-quality` job) | ⚠️ Runs, findings ignored | Confirmed via direct local run: genuinely resolves and executes, reports 8,305 violations, all silently absorbed — tracked as #354 |
