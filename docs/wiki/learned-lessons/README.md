...
| [checkstyle-ratchet-counts-whole-file-not-diff-add-javadoc-per-new-method.md](checkstyle-ratchet-counts-whole-file-not-diff-add-javadoc-per-new-method.md) | CheckStyle's `maxAllowedViolations` ratchet counts the whole compiled tree, not the diff — new code matching an already-violating sibling's style still adds new occurrences and can push the total over the ceiling; write clean javadoc/final-params/line-length for new code regardless of surrounding debt, verify locally before pushing (#84, PR #370) | technical | 2026-07-13 |
