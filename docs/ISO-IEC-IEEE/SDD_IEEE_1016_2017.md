...
- Security chain (JWT authentication, RBAC, rate limiting)
- External integrations (MySQL, Redis, Elasticsearch, Razorpay)
- Resilience patterns (circuit breakers, time limiters)
- Event-driven design (domain events, webhooks)
- Deployment topology (Docker, Kubernetes, AWS Terraform)

**Out of Scope:**

- Frontend/client applications
- Third-party service internals (Razorpay, infrastructure providers)
...
```mermaid
flowchart LR
  Client["API Clients<br/>(Web/Mobile)"] -->|HTTP/REST| API["BuildNest<br/>Spring Boot API<br/>:8080"]

  API -->|JDBC/HikariCP| MySQL[("MySQL 8.2<br/>:3306")]
  API -->|Jedis| Redis[("Redis 7<br/>:6379")]
  API -->|REST Client| ES[("Elasticsearch 8.10<br/>:9200")]
  API -->|REST SDK| Razorpay["Razorpay<br/>Payment Gateway"]

  API -->|Micrometer| Prometheus["Prometheus<br/>Monitoring"]
  API -->|TCP/JSON| Logstash["Logstash<br/>:5000"]
  Logstash --> ES
  ES --> Kibana["Kibana<br/>Dashboard"]
...
├── annotation/         # Custom annotations (@RateLimit, etc.)
├── aspect/             # AOP aspects (logging, auditing)
├── config/             # Spring configuration classes (38 files)
│   └── properties/     # @ConfigurationProperties beans (8 files)
├── controller/         # REST controllers (28 total)
│   ├── admin/          # Admin endpoints (14 controllers)
│   ├── auth/           # Authentication endpoints (2 controllers)
│   ├── inventory/      # Inventory status endpoints (1 controller)
│   ├── monitoring/     # Metrics endpoints (2 controllers)
│   ├── public_/        # Public endpoints (1 controller)
│   └── user/           # User-facing endpoints (8 controllers)
├── event/              # Domain events (8 files)
├── exception/          # Exception hierarchy (11 files)
├── integration/        # External system adapters (RazorpayClientAdapter)
├── interceptor/        # HTTP interceptors
├── model/              # Domain model (48 total)
│   ├── dto/            # Data Transfer Objects (13 DTOs)
│   ├── elasticsearch/  # ES document models (2 files)
│   ├── entity/         # JPA entities (18 entities)
│   └── payload/        # Request/Response payloads (11 files)
├── monitoring/         # Custom monitoring components
├── repository/         # Spring Data JPA repositories (19 total)
│   └── elasticsearch/  # ES repositories (2 files)
├── security/           # Security filters and JWT (8 files)
│   └── Jwt/            # JWT provider, filter, entry point
└── service/            # Business logic (56 total)
    ├── admin/          # Admin service (4 files)
    ├── analytics/      # Sales analytics (2 files)
    ├── audit/          # Audit logging (2 files)
    ├── auth/           # Authentication (2 files)
    ├── cart/           # Cart management (2 files)
    ├── category/       # Category management (2 files)
    ├── checkout/       # Checkout processing (2 files)
    ├── elasticsearch/  # ES operations (5 files)
    ├── inventory/      # Inventory management (5 files)
    ├── monitoring/     # Performance monitoring (3 files)
    ├── notification/   # Notification service (2 files)
    ├── order/          # Order management (2 files)
    ├── password/       # Password reset (2 files)
    ├── payment/        # Payment processing (3 files)
    ├── product/        # Product management (2 files)
    ├── ratelimit/      # Rate limiting (2 files)
    ├── review/         # Product reviews (2 files)
    ├── scheduler/      # Background schedulers (2 files)
    ├── token/          # Refresh token management (2 files)
    ├── user/           # User profile (2 files)
    └── webhook/        # Webhook dispatching (2 files)
```
...
flowchart TB
  subgraph "Presentation Layer"
    direction LR
    AuthCtrl["Auth<br/>Controllers<br/>(2)"]
    UserCtrl["User<br/>Controllers<br/>(8)"]
    AdminCtrl["Admin<br/>Controllers<br/>(14)"]
    MonCtrl["Monitoring<br/>Controllers<br/>(2)"]
    InvCtrl["Inventory<br/>Controllers<br/>(1)"]
    PubCtrl["Public<br/>Controllers<br/>(1)"]
  end
  ...
  subgraph "Business Logic Layer"
    direction LR
    AuthSvc["Auth & Token<br/>Services"]
    CartSvc["Cart<br/>Service"]
    CheckoutSvc["Checkout<br/>Service"]
    PaymentSvc["Payment<br/>Service"]
    ProductSvc["Product & Category<br/>Services"]
    InventorySvc["Inventory<br/>Services"]
    OrderSvc["Order<br/>Service"]
    ReviewSvc["Review & Wishlist<br/>Services"]
    AuditSvc["Audit & Analytics<br/>Services"]
    WebhookSvc["Webhook<br/>Service"]
  end

  subgraph "Data Access Layer"
    direction LR
    JPA["Spring Data JPA<br/>Repositories (17)"]
    ESRepo["Elasticsearch<br/>Repositories (2)"]
  ...
  AuthCtrl & UserCtrl & AdminCtrl & MonCtrl & InvCtrl & PubCtrl --> JwtFilter & RateLimit
  JwtFilter & RateLimit --> AuthSvc & CartSvc & CheckoutSvc & PaymentSvc & ProductSvc & InventorySvc & OrderSvc & ReviewSvc & AuditSvc & WebhookSvc
  AuthSvc & CartSvc & CheckoutSvc & PaymentSvc & ProductSvc & InventorySvc & OrderSvc & ReviewSvc & AuditSvc --> JPA & ESRepo & Cache
  PaymentSvc --> RZP
  JPA --> MySQL
  Cache --> Redis
  ESRepo --> ES
...
| ------------------------ | -------------------------------------------------------------------- | ----------- |
| FR-AUTH-01 to FR-AUTH-11 | `AuthServiceImpl`, `JwtTokenProvider`, `SecurityConfig`              | §4.3, §5.1  |
| FR-PROD-01 to FR-PROD-07 | `ProductServiceImpl`, `CategoryServiceImpl`, `CacheConfig`           | §4.3, §4.5  |
| FR-CART-01 to FR-CART-06 | `CartServiceImpl`, `CartController`                                  | §4.3, §4.7  |
| FR-CHK-01 to FR-CHK-08   | `CheckoutServiceImpl`, `CheckoutController`                          | §4.3, §4.8  |
| FR-PAY-01 to FR-PAY-05   | `PaymentServiceImpl`, `RazorpayClientAdapter`                        | §4.3, §4.8  |
| FR-INV-01 to FR-INV-07   | `InventoryServiceImpl`, `InventoryMonitoringScheduler`               | §4.3, §4.9  |
| FR-MON-01 to FR-MON-08   | Health indicators, `PerformanceMonitoringService`, Prometheus config | §4.10       |
| SEC-01 to SEC-13         | `SecurityConfig`, `JwtTokenProvider`, `RateLimiterService`           | §5.1        |
| REL-01 to REL-05         | `ResilienceConfig`, circuit breakers, time limiters                  | §5.3        |
| PR-01 to PR-08           | HikariCP config, Redis config, response time targets                 | §4.10       |
| DC-01 to DC-06           | Package structure, Docker, K8s manifests, graceful shutdown          | §4.2, §4.10 |
