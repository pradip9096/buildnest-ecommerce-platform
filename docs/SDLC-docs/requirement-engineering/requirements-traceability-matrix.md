...
| FR-SEL-06 | Seller views/manages orders containing their own products only | High | Ph-3 | §4.5.1 | `OrderGroup` entity + `Order.orderGroup` (nullable FK) schema for splitting a multi-seller cart into per-seller orders (#578, sub-issue of #557). Checkout-split (#579) and seller-scoped order API (#580) not yet implemented | `OrderRepositoryTest` (H2 FK round-trip) | Test | 🟡 In Progress |
