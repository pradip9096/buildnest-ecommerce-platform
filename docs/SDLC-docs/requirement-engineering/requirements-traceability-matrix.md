...
| **Related SRS** | SRS-BUILDNEST-001 v5.18 — `docs/SDLC-docs/requirement-engineering/software-requirements-specification.md` |
...
| 1.61 | 2026-08-09 IST | QA Manager | Updated MNT-07 (#127) from 🔵 Pending to ✅ Implemented after live post-merge verification: an initial `workflow_dispatch` run (31279941003) failed fetching `/v3/api-docs.yaml` (curl exit 22 — `SecurityConfig.java`'s `/v3/api-docs/**` Ant pattern doesn't match the `.yaml` suffix variant, only sub-paths, so the request fell through to `anyRequest().authenticated()` and was rejected unauthenticated); fixed in self-correction PR #713 by switching to the already-public `/v3/api-docs` JSON endpoint (no security-config change needed). Re-run (31280759257) succeeded end-to-end; `https://pradip9096.github.io/buildnest-ecommerce-platform/` confirmed returning HTTP 200 with a real OpenAPI 3.1.0 spec (159 paths). Updated the Coverage Summary row "Maintainability (MNT)" (7/6/0/1/0/0 → 7/7/0/0/0/0) and the Totals row (147/44 → 148/43, Implemented/Pending) | Pending |
| 1.62 | 2026-08-09 IST | QA Manager | Added COMP-04 (#129, WCAG 2.1 AA frontend accessibility audit + remediation) — the issue was titled "(COMP-02)" and cited "SRS NFR-COMP-04", but COMP-02 is the unrelated GDPR account-deletion requirement and no COMP-04/NFR-COMP-* row existed at filing time — same filing-time traceability-mismatch shape as MNT-07/OBS-05/COMP-01/RET-01/FR-AUTH-12. ✅ Implemented: real-browser `@axe-core/playwright` scan (WCAG 2.1 A/AA tag set) across all 12 customer-facing pages plus a keyboard-navigation and a screen-reader structural proxy test (`frontend/e2e/accessibility.spec.ts`, 15 specs) — zero critical/serious violations after fixing 6 distinct color-contrast defects (`text-gray-400`/`text-green-600`/`text-red-500`/`text-amber-600` below 4.5:1 across Login/Register/ProductDetail/Account/CheckoutStepper/Cart/Listing/Reviews/OrderConfirmation), 2 invalid `autocomplete` tokens (`RegisterPage.tsx`), and one missing `id`/`htmlFor` label association (`AddressStep.tsx`, the same defect class #286 fixed on Login/Account). See `docs/SDLC-docs/reports/accessibility-audit.md` for the full audit, including explicitly disclosed proxy-vs-real-AT limitations (no real screen-reader binary or manual QA pass available in this environment). Added new Coverage Summary row entry (Compliance (COMP) 3/3/0/0/0/0 → 4/4/0/0/0/0) and recomputed the Totals row (205→206 total, 148→149 Implemented). Updated `Related SRS` to v5.17 | Pending |
| 1.63 | 2026-08-14 IST | QA Manager | Added §7.6 Maintainability Requirements row MNT-08 (#134, frontend SEO metadata + sitemap generation) — the issue's own "SRS NFR-MAINT-05" citation referenced an ID that did not exist at filing time (this section uses the `MNT-*` prefix, not `NFR-MAINT-*`, and MNT-05 already covers an unrelated requirement — structured logging), same filing-time traceability-mismatch shape as MNT-07/COMP-04/OBS-05/COMP-01/RET-01/FR-AUTH-12. Adopted `MNT-08`, the next free ID in the existing sequence. 🔵 Pending — `SeoMeta` component/`HelmetProvider`/sitemap generator/`robots.txt` implemented and unit-tested (`SeoMeta.test.tsx`, `generate-sitemap.test.mjs`), but the Lighthouse SEO ≥ 90 acceptance criterion itself not yet live-verified against a deployed instance (no production/staging deployment target exists yet). Updated the Coverage Summary row "Maintainability (MNT)" (7/7/0/0/0/0 → 8/7/0/1/0/0) and recomputed the Totals row (206→207 total, 43→44 Pending). Updated `Related SRS` to v5.18 | Pending |
...
| Compliance (COMP) | 4 | 4 | 0 | 0 | 0 | 0 |
| Maintainability (MNT) | 8 | 7 | 0 | 1 | 0 | 0 |
...
| COMP-04 | Frontend shall conform to WCAG 2.1 Level AA — automated scan zero critical/serious violations, ≥4.5:1 text contrast, keyboard-operable checkout, labeled/landmarked forms and pages | High | Ph-2 | §3.8.4 | `frontend/e2e/accessibility.spec.ts` (`@axe-core/playwright`, WCAG 2.1 A/AA tags), color-contrast fixes across `LoginPage.tsx`/`RegisterPage.tsx`/`ProductDetailPage.tsx`/`AccountPage.tsx`/`CheckoutStepper.tsx`/`ProductGrid.tsx`/`ProductCard.tsx`/`CategorySidebar.tsx`/`ReviewsSection.tsx`/`OrderConfirmationPage.tsx`/`CartPage.tsx`, `RegisterPage.tsx` autocomplete-token fix, `AddressStep.tsx` `id`/`htmlFor` label association fix | `frontend/e2e/accessibility.spec.ts` (15 specs: 12-page axe scan, keyboard-nav proxy, screen-reader structural proxy) | Test | ✅ Implemented |
...
| MNT-07 | Generated OpenAPI 3.1 spec published as browsable API docs (Swagger UI) via GitHub Pages, kept current on each release tag | Medium | Ph-5 | N/A | `.github/workflows/publish-api-docs.yml` | N/A — tier-0 CI-config change, verified via real workflow execution + live Pages URL check, not a test class | Inspection | ✅ Implemented (live-verified 2026-08-08: `workflow_dispatch` run succeeded end-to-end, `https://pradip9096.github.io/buildnest-ecommerce-platform/` returns HTTP 200 with a real OpenAPI 3.1.0 spec, 159 paths) |
| MNT-08 | Public-facing frontend pages carry correct SEO metadata (title, meta description, OG, Twitter Card) and a build-time-generated `sitemap.xml`/`robots.txt`, achieving Lighthouse SEO ≥ 90 | Medium | Ph-5 | N/A | `frontend/src/components/common/SeoMeta.tsx`, `frontend/src/main.tsx` (`HelmetProvider`), `frontend/scripts/generate-sitemap.mjs` (postbuild), `frontend/public/robots.txt` | `SeoMeta.test.tsx`, `generate-sitemap.test.mjs` | Inspection | 🔵 Pending — title/meta/OG/Twitter Card tags and sitemap/robots.txt generation implemented and unit-tested; the Lighthouse SEO ≥ 90 acceptance criterion itself not yet live-verified against a deployed instance (no production/staging deployment target exists yet, per `development-workflow.md` step 31) |
