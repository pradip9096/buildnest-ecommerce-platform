...
| FR-SEL-06 | Seller views/manages orders containing their own products only | High | Ph-3 | §4.5.1 | `OrderGroup` entity + `Order.orderGroup` schema (#578), checkout-time order splitting in `CheckoutServiceImpl` (#579), and `SellerOrderController`/`OrderServiceImpl` seller-scoped list/detail/status endpoints via `OrderRepository.findBySellerId`/`findByIdAndSellerId` (#580, sub-issues of #557) | `OrderRepositoryTest` (H2 FK round-trip; seller-scoping `EXISTS`-subquery); `CheckoutServiceImplTest` (seller-split unit tests); `OrderServiceImplTest`/`SellerOrderControllerTest` (seller ownership/transition/status unit tests) | Test | ✅ Implemented |
