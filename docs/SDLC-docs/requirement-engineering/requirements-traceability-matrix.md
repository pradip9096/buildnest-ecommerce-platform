...
| FR-SEL-06 | Seller views/manages orders containing their own products only | High | Ph-3 | §4.5.1 | `OrderGroup` entity + `Order.orderGroup` schema (#578) and checkout-time order splitting in `CheckoutServiceImpl` (#579, sub-issues of #557). Seller-scoped order API (#580) not yet implemented | `OrderRepositoryTest` (H2 FK round-trip); `CheckoutServiceImplTest` (seller-split unit tests) | Test | 🟡 In Progress |
